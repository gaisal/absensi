            ...
            if ($row['username']=="Name") {
                ...
                if (!empty($Uname) && !empty($Number) && !empty($Email)) {
                    ...
                    if (!mysqli_stmt_prepare($result, $sql)) {
                        ...
                        $resultl = mysqli_stmt_get_result($result);
                        if (!$row = mysqli_fetch_assoc($resultl)) {
                            $sql="UPDATE users SET username=?, serialnumber=?, gender=?, kelas=?, jurusan=?, email=?, user_date=CURDATE(), time_in=? WHERE fingerprint_select=1";
                            $result = mysqli_stmt_init($conn);
                            if (!mysqli_stmt_prepare($result, $sql)) {
                                echo "SQL_Error_select_Fingerprint";
                                exit();
                                ...
                                mysqli_stmt_bind_param($result, "sdsssss", $Uname, $Number, $Gender,$Kelas, $Jurusan, $Email, $Timein );
                                mysqli_stmt_execute($result);
                        ...
                        if (!$row = mysqli_fetch_assoc($resultl)) {
                            ...
                            if (!mysqli_stmt_prepare($result, $sql)) {
                              ...
                              exit();
                                ...
                                mysqli_stmt_execute($result);
                                $sql = "INSERT INTO users ( username, serialnumber, gender, kelas, jurusan, email, fingerprint_id, fingerprint_select, user_date, time_in, del_fingerid , add_fingerid) VALUES (?, ?, ?, ?, ?, ?, ?, 1, CURDATE(), ?, 0, 1)";
                                $result = mysqli_stmt_init($conn);
                                if (!mysqli_stmt_prepare($result, $sql)) {
                                  echo "SQL_Error";
                                  exit();
                                    ...
                                    mysqli_stmt_bind_param($result, "sdssssis", $Uname, $Number, $Gender, $Kelas, $Jurusan, $Email, $fingerid, $Timein );
                                    mysqli_stmt_execute($result);
      ...
      exit();
        ...
        if ($row = mysqli_fetch_assoc($resultl)) {
            ...
            if (empty($row['username'])) {
                ...
                if (empty($Uname) && empty($Number) && empty($Email) && empty($Timein) && empty($Kelas) && empty($Jurusan)) {
                    echo "Empty Fields";
                    exit();
                }
                else{
                    ...
                    if (!mysqli_stmt_prepare($result, $sql)) {
                        ...
                        if (!$row = mysqli_fetch_assoc($resultl)) {
                            ...
                            if (!empty($Uname) && !empty($Email) && !empty($Timein) && !empty($Kelas) && !empty($Jurusan)) {

                                $sql="UPDATE users SET username=?, serialnumber=?, gender=?,  kelas=?, jurusan=?,  email=?, time_in=? WHERE fingerprint_select=1";
                                $result = mysqli_stmt_init($conn);
                                if (!mysqli_stmt_prepare($result, $sql)) {
                                    echo "SQL_Error_select_Fingerprint";
                                    exit();
